...
/**
 * Q: What is it?
 * A: A singleton for reasoning about static class hierarchy of the given application, esp. w.r.t. special GUI interfaces:
 *
...
//        }
    ...
    private Hierarchy() {
        ...
        Logger.trace("TIMECOST", "HIER parsing finished at " + System.currentTimeMillis());
    } // Hierarchy()
    ...
    void simpleClassStatistics() {
        ...
        Logger.verb(this.getClass().getSimpleName(),
                "[HIER] All classes: " + numClasses);

        Logger.verb(this.getClass().getSimpleName(),
                "[App: " + appClasses.size() + ", Lib : "
                        + scene.getLibraryClasses().size()
                        + ", Phantom: " + scene.getPhantomClasses().size() + "]");
    ...
    void recordFrameworkInvokedCallbacks() {
        for (SootClass c : appClasses) {
            ...
            if (instantiated && !applicationActivityClasses.contains(c)) {
                continue;
            }
    ...
    void activityClasses() {
        ...
        Logger.verb(this.getClass().getSimpleName(),
                "[HIER] Activities: " + applicationActivityClasses.size()
    ...
    void viewsAndMenus() {
        ...
        Logger.verb(this.getClass().getSimpleName(),
                "[HIER] App views: " + numAppViews + ", Lib views: " + numLibViews);
    ...
    void dialogs() {
        ...
        Logger.verb(this.getClass().getSimpleName(),
                "[HIER] App Dialogs: " + applicationDialogClasses.size()
...
//    }
        if (instantiatedClasses == null)
            buildApplicationCodeMap();
        return instantiatedClasses.contains(c);
    }

    private Set<SootClass> instantiatedClasses;

    void buildApplicationCodeMap() {
        Logger.trace("TIMECOST", "buildApplicationCodeMap parsing starting at " + System
                .currentTimeMillis());
        ...
        for (SootClass d : appClasses) {
            for (SootMethod m : Lists.newArrayList(d.getMethods())) {
                ...
                Body b = m.retrieveActiveBody();
                for (Iterator<Unit> iter = b.getUnits().snapshotIterator();
                     iter.hasNext(); ) {
                    ...
                    Value rhs = ((AssignStmt) stmt).getRightOp();
                    if (rhs instanceof NewExpr) {
                        SootClass base = ((NewExpr) rhs).getBaseType().getSootClass();
                        instantiatedClasses.add(base);
                    }
                }
            }
        }
        Logger.trace("TIMECOST", "buildApplicationCodeMap parsing finished at " + System
                .currentTimeMillis());
    }
